var supl = {
	start_up : function	() {
		...
		$('[name=npwp_supl]').mask("99.999.999.9-999.999");
		...
		if (error > 0) {
			...
			bootbox.confirm('Apakah anda yakin data mitra akan disimpan?', function(result){
    			if (result) {
        			...
        			var lampirans = $.map( $(div_supplier).find('input[type=file]'), function(ipt){
			            if (!empty( $(ipt).val() )) {
					...
					formData.append('data_supplier', JSON.stringify(data_supplier));
					supl.executeSave(formData);
