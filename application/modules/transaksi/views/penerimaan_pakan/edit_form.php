    ...
    <div class="col-xs-6 d-flex align-items-center no-padding filter">
            ...
            <select class="form-control unit" data-required="1">
                ...
                <?php if ( count($unit) > 0 ): ?>
                    <?php foreach ($unit as $k => $val): ?>
                        <?php 
                            $true = false;
                            if ( stristr($data['kirim_pakan']['no_order'], $val['kode']) !== FALSE ) { 
                                $true = true;
                            }
                        ?>
        ...
        <div class="col-xs-3" style="padding-left: 0px;">
            <div class="input-group date datetimepicker" name="tgl_kirim" id="tgl_kirim">
                <input type="text" class="form-control text-center" placeholder="Tanggal Kirim" data-required="1" data-tgl="<?php echo $data['kirim_pakan']['tgl_kirim']; ?>" />
...
<div class="form-group d-flex align-items-center">
    <div class="col-xs-6 d-flex align-items-center no-padding">
        ...
        <div class="col-xs-6">
            <select class="form-control no_sj" data-required="1" onchange="pp.get_data_by_sj(this)" data-id="<?php echo $data['kirim_pakan']['id']; ?>" data-nosj="<?php echo $data['kirim_pakan']['no_sj']; ?>">
                <option value="">-- Pilih No. SJ --</option>
                <!-- <?php if ( count($get_sj_not_terima) > 0 ): ?>
                	<?php foreach ($get_sj_not_terima as $k => $val): ?>
                        <?php
                            $selected = null;
                            if ( $val['id'] == $data['kirim_pakan']['id'] ) {
                                $selected = 'selected';
                            }
                        ?>
                	...
                	<?php endforeach ?>
                <?php endif ?>
                <option value="<?php echo $data['kirim_pakan']['id'] ?>" selected ><?php echo $data['kirim_pakan']['no_sj']; ?></option> -->
            </select>
        </div>
    ...
    <div class="col-xs-6 d-flex align-items-center no-padding">
        ...
        <div class="col-xs-4">
            <input type="text" class="form-control no_pol" placeholder="No. Polisi" data-required="1" value="<?php echo $data['kirim_pakan']['no_polisi'] ?>" readonly>
        </div>
    </div>
</div>
<div class="form-group d-flex align-items-center">
    <div class="col-xs-6 d-flex align-items-center no-padding">
        ...
        <div class="col-xs-8">
            <input type="text" class="form-control ekspedisi" placeholder="Ekspedisi" data-required="1" value="<?php echo $data['kirim_pakan']['ekspedisi'] ?>" readonly>
        </div>
    </div>
    <div class="col-xs-6 d-flex align-items-center no-padding">
        <div class="col-xs-2 text-left">Sopir</div>
        <div class="col-xs-4">
            <input type="text" class="form-control sopir" placeholder="Sopir" data-required="1" value="<?php echo $data['kirim_pakan']['sopir'] ?>" readonly>
        </div>
    </div>
</div>
<div class="form-group d-flex align-items-center">
    <div class="col-xs-6 d-flex align-items-center no-padding">
        ...
        <div class="col-xs-4">
            <input type="text" class="form-control jenis_kirim" placeholder="Jenis" data-required="1" value="<?php echo $data['kirim_pakan']['jenis_kirim'] ?>" readonly>
        </div>
    </div>
    <div class="col-xs-6 d-flex align-items-center no-padding">
        <div class="col-xs-2 text-left">No. Order</div>
        <div class="col-xs-4">
            <input type="text" class="form-control no_order" placeholder="No. Order" data-required="1" value="<?php echo $data['kirim_pakan']['no_order'] ?>" readonly>
        </div>
    </div>
</div>
<div class="form-group d-flex align-items-center">
    <div class="col-xs-6 d-flex align-items-center no-padding">
        ...
        <div class="col-xs-4">
            <input type="text" class="form-control tgl_kirim" placeholder="Tanggal" data-required="1" value="<?php echo tglIndonesia($data['kirim_pakan']['tgl_kirim'], '-', ' ') ?>" readonly>
        </div>
    </div>
    <div class="col-xs-6 d-flex align-items-center no-padding">
            ...
            <tbody>
                <?php foreach ($data['detail'] as $k_det => $v_det): ?>
                    <?php
                        $jml_kirim = 0;
                        $kondisi_kirim = '';
                        foreach ($data['kirim_pakan']['detail'] as $kp_det => $vp_det) {
